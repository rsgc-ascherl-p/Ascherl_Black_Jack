...
int ps; //Value of player's cards added together
int hs; //Value of house's cards added together

int[] ph = new int[5]; //Declare Player Hand Array
int[] hh = new int[5]; //Declare House Hand Array

int s1; //Prevent further looping for balance to not continuously going up or down (Text messages)
...
void draw () {
  ...
  //Text  of the player's cards added up
  ps = p1 + p2 + p3 + p4 + p5;
  ...
  fill(0);
  textSize(30);
  text("Player Hand:" + ps, 0, 30);

  //Text of the house's cards being added up
  hs = h1 + h2 + h3 + h4 + h5;
...
void mouseClicked () {
  ...
  //Redeal button, resets everything for a new round
  if (mouseX > 50 && mouseX < 100 && mouseY > 325 && mouseY < 375 && gm == 0) {
    ...
    //Reset all Card values so the score counter doesn't go above 21
    p1 = 0;
    p2 = 0;
    p3 = 0;
    p4 = 0;
    ...
    //Set value counter for next round
    ps = p1 + p2 + p3 + p4 + p5;
    hs = h1 + h2 + h3 + h4 + h5;
    ...
    //Clear the player's score value
    fill(170);
    rect(0, 0, 249, 40);
...
void keyPressed () {
  if (key == 'r' && gm == 1) {
    ...
    //Reset the balance aspect of the game
    cash = 25;
    gm = 0;
